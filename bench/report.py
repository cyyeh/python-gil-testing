...
def _marker(kind: str, x: float, y: float, cls: str) -> str:
    r = 4.5
    if kind == "circle":
...
def line_chart_svg(series: dict, xs: list, title: str, y_label: str = "median wall time (s) - lower is better",
    ...
    for i, (label, pts) in enumerate(series.items()):
        ...
        for x in xs:
            v = pts.get(x)
            if v is not None:
                parts.append(_marker(_MARKERS[i % 4], xpos[x], ypos(v), cls))
                last = (xpos[x], ypos(v))
        if last:
    ...
    parts.append("</svg>")
    # legend (always present for >= 2 series) lives in HTML below the plot so it can never collide with labels
    legend = "".join(
    ...
    for c in configs:
        ...
        for w in workers:
            ...
            if rec is None:
                ...
                cells.append(f'<td title="{html.escape(rec["skipped"])}">skipped</td>')
            elif rec.get("error"):
...
def _config_table(doc: dict) -> str:
    ...
    for c in doc["configs"]:
        ...
        rows.append(f"<tr><th scope=\"row\">{html.escape(c['label'])}</th><td>{html.escape(str(info.get('version', '?')))}</td>"
...
def _facts_table(doc: dict) -> str:
    ...
    return (f'<table class="data facts"><thead><tr><th>measured fact</th>{head}</tr></thead><tbody>{"".join(rows)}</tbody></table>')
...
nav.toc a { color: var(--text-2); text-decoration: none; border-bottom: 1px solid var(--line); }
...
svg.chart { width: 100%; max-width: 680px; height: auto; display: block; font-family: inherit; }
...
.tooltip { position: absolute; pointer-events: none; background: var(--surface); color: var(--text); border: 1px solid var(--line); border-radius: 6px; padding: 6px 10px; font-size: 12px; box-shadow: 0 2px 8px rgba(0,0,0,.12); min-width: 150px; }
...
@media (max-width: 600px) { h1 { font-size: 24px; } table.data { font-size: 12.5px; } }
"""
...
(function () {
  ...
  document.querySelectorAll('.chart-wrap').forEach(function (wrap) {
    ...
    var svg = wrap.querySelector('svg'), hit = wrap.querySelector('.hit'),
        cross = wrap.querySelector('.crosshair'), tip = wrap.querySelector('.tooltip');
    ...
    function show(ev) {
      ...
      data.xpos.forEach(function (px, i) { var d = Math.abs(px - x); if (d < bd) { bd = d; best = i; } });
      cross.setAttribute('x1', data.xpos[best]); cross.setAttribute('x2', data.xpos[best]);
      ...
      tip.hidden = false;
      var left = (data.xpos[best] / sx) + 12, top = (ev.clientY - r.top) - 10;
      if (left + tip.offsetWidth > r.width) left = (data.xpos[best] / sx) - tip.offsetWidth - 12;
...
<div class="table-scroll">{_config_table(doc)}</div>
<h4>Speed-up at a glance</h4>
<div class="table-scroll">{_summary_table(doc, p, configs)}</div>
